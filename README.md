...
ブラウザで動くワークアウト・インターバルタイマーです。`timeline.md` に定義したメニュー（スロースクワット → ヒップヒンジ → その場足踏み／ニーアップ → 休憩 を3周＋整理運動、計10分）を、音と画面表示でガイドします。
...
[Corepack](https://nodejs.org/api/corepack.html) を有効にして、ピン留めされた pnpm を使ってください（グローバルインストールは不要）。
